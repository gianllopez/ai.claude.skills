...
**Impact (HIGH):** Centralizing API logic ensures consistent caching strategies, type safety across network boundaries, and unified authentication handling. The "Safe Return" pattern prevents UI crashes due to backend failures.
...
2.  **Directory Structure:**
    ...
    - Files should be named `use-<action>.ts` (e.g., `use-assets.ts`, `use-create-asset.ts`).
3.  **Type Definitions:**
    - Define `QueryResponse` (_API_ Contract), `QueryData` (UI consumption), and `QueryVariables`.
    - Return types must be explicit.
4.  **Query Keys:**
    - Format: `['@<domain>/<hook-name>', variables]`.
    - Example: `['@users/use-assets', id]`.
