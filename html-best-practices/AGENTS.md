...
> generating, or refactoring HTML, CSS, and TailwindCSS v4 codebases. Humans
...
Standards for the part of a web _UI_ that is the same regardless of the meta-framework rendering it: the markup that carries the meaning and the _TailwindCSS_ v4 theme that presents it. Semantic elements first, because the element is what makes a page machine-readable and its structure legible; then _TailwindCSS_ v4, where styling resolves through theme tokens rather than arbitrary values, sized to the project — one plain theme by default, the variable-backed semantic layer only where a component generator or a theme swap reads it; then the two defects utility CSS makes easiest to ship, layout shift and a stylesheet that never saw half its classes. Framework-specific concerns — forms libraries, component generators, compiler behavior — live in `react-best-practices` and `astro-best-practices`, which load this skill alongside their own. Accessibility auditing is excluded by design.
...
1. [Semantic Markup](#1-semantic-markup) — `CRITICAL`
   ...
   - [1.3 Content Elements: Lists, Tables & Media](#13-content-elements-lists-tables--media)
...
3. [Performance & Robustness](#3-performance--robustness) — `HIGH`
   - [3.1 Layout Stability & Overflow](#31-layout-stability--overflow)
   - [3.2 Source Detection & CSS Footprint](#32-source-detection--css-footprint)
...
### 1.3 Content Elements: Lists, Tables & Media
...
2.  **Size the token layer to the project:**
    ...
    - The second, variable-backed layer — `:root` / `.dark` pairs mapped in through `@theme inline` — solves exactly one problem: a value that has to change while the same class stays on the element. A component generator such as `shadcn/ui` needs it, because its components are generated against those variable names, and a theme swap needs it — dark mode, per-tenant branding. Nothing else does
...
3.  **Where a component generator such as `shadcn/ui` is installed, the semantic layer is not optional decoration:**
    ...
    - The brand colour is declared there and nowhere else: hand-written components paint with `bg-primary` too, which is the only thing that keeps them in step with every generated one
    ...
    - A plain `@theme` sits beside it for what the generator does not decide: the font, type steps, radii of the project's own
...
4.  **Reach for the token that names the role, where the role has a token:**
...
7.  **Do not bypass the utility layer:**
    ...
    - The `style` attribute is reserved for values computed at runtime (an animated transform, a measured offset)
8.  **v4 configuration:**
...
```html
<!-- Bad: brand color and spacing invented at the call site, and the token
     that exists is reached through style, which skips the utility layer -->
<span
...
```html
<span class="rounded-badge bg-brand px-3 py-1 text-badge text-white">Beta</span>
```

**Correct — with a semantic layer, where the variable-backed indirection earns its place:**
...
/* Written by a component generator such as shadcn init: the semantic layer
   every generated component reads, and the only place the brand colour is
   declared. Each surface ships with the foreground that is legible on it */
:root {
  --background: oklch(1 0 0);
  --foreground: oklch(0.145 0 0);
...
```html
<span
  class="rounded-badge bg-primary px-3 py-1 text-badge text-primary-foreground"
>
  Beta
</span>
```
...
**Impact (MEDIUM):** Sorting is not what settles a conflict — the stylesheet is, which is the whole reason a merge helper exists — so order itself is never worth a review comment once the formatter runs. What lifts this above cosmetics is everything around it: a plugin that never sees a class-merging helper leaves every composed string unsorted, contradictory utilities left inside one string resolve by stylesheet order instead of by intent, and a formatter let loose on generated folders fills diffs with noise that hides the real edits.
...
2.  **Configure it for composed strings:**
    ...
    - List class-merging helper functions in `tailwindFunctions` (`cn`, `cva`, `clsx`, …) and custom attributes in `tailwindAttributes`, otherwise those strings go unsorted
    ...
    - A directory of generated component code stays in `.prettierignore`, so the formatter never rewrites it and any diff there is a deliberate edit
...
```js
// ./prettier.config.js
// Bad: no stylesheet reference and no helper functions — merged class strings never get sorted
export default { plugins: ['prettier-plugin-tailwindcss'] };
```

```html
<!-- Bad: hand-grouped into blocks the plugin can no longer sort against each
     other, with contradictory utilities and a leftover from a previous layout -->
<div class="block flex flex-col md:flex-row p-6 p-4 text-sm">...</div>
```
...
<div class="flex flex-col gap-4 p-4 text-sm text-muted-foreground md:flex-row">
  ...
</div>
```
...
### 3.2 Source Detection & CSS Footprint
